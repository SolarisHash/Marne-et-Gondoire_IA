...
class MGOpenAIClient:
    """
    Client pour interagir avec l'API OpenAI et le serveur MCP.
    """
    
    def __init__(self, api_key: str = None, mcp_url: str = None, model: str = "gpt-4o"):
        """
        Initialise le client OpenAI avec la clé API et l'URL du serveur MCP.
        
        Args:
            api_key: La clé API OpenAI (par défaut: utilise la variable d'environnement)
    ...
    def chat(self, prompt: str, system_message: str = None) -> Dict[str, Any]:
        """
        Envoie une requête de chat au modèle OpenAI avec accès au serveur MCP.
        ...
        try:
            ...
            if message.tool_calls:
                ...
                for tool_call in message.tool_calls:
                    ...
                    # Ici, nous simulerons l'appel au serveur MCP
                    # Dans un cas réel, vous feriez une requête HTTP au serveur MCP
                ...
                messages = [
                    {"role": "system", "content": system_message},
                    {"role": "user", "content": prompt},
                    message,  # La réponse initiale du modèle avec les appels d'outils
                ]
                ...
                for i, tool_call in enumerate(message.tool_calls):
                    messages.append({
                        ...
                        "content": json.dumps(tool_responses[i])
                    })
                ...
                return {
                    ...
                    "tool_calls": [
                        ...
                        for tc, resp in zip(message.tool_calls, tool_responses)
                    ]
                }
...
if __name__ == "__main__":
    ...
    response = client.chat("Quelle est la tendance du nombre de permis de construire pour les trois prochains mois?")
